...
public class initAclCollections {
    ...
    public void initPermissionCollection(MongoTemplate template) {
        ...
        ObjectPermission p2 = template.save(new ObjectPermission(ADMIN, 2, true, false, false));
        ObjectPermission p3 = template.save(new ObjectPermission(new MongoSid("dima"), 1, true, false, false));
        fillPermissionList(p1, p2, p3);
    }
    ...
    public void initAclCollection(MongoTemplate template) {
        ...
        acl1.getPermissions().add(permissions.get(0));
        acl2.getPermissions().add(permissions.get(1));
        acl3.getPermissions().add(permissions.get(2));
        template.save(acl1);
        template.save(acl2);
        template.save(acl3);
    }
